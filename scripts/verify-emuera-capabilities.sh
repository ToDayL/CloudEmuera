...
expected_matrix = "p1-s10"
expected_runtime = "headless-p0.5.3"
expected_protocol = 8
expected_digest = hashlib.sha256(
    f"cloudemuera:{expected_matrix}:{expected_upstream}:structured-console-v8-button-generation".encode()
).hexdigest()
...
if matrix.get("matrixVersion") != expected_matrix:
    fail("matrixVersion is not p1-s10")
...
if matrix.get("protocolVersion") != expected_protocol:
    fail("structured protocol version is not 8")
...
if "CurrentVersion = 8" not in protocol or "CapabilityMatrixVersion = \"p1-s10\"" not in protocol:
    fail("v8 structured IPC constants are not frozen")
proto = (root / "src/CloudEmuera.Ipc/Protos/structured-worker.proto").read_text(encoding="utf-8")
for required_proto in ("package cloudemuera.ipc.v8;", "string capability_set_digest", "message ConsoleSnapshot", "message ConsoleTransaction", "message TooltipPresentation", "message TooltipResource", "int64 button_generation"):
    if required_proto not in proto:
        fail(f"structured-worker.proto misses {required_proto}")

worker_manager = (root / "src/CloudEmuera.Api/Workers/WorkerManager.cs").read_text(encoding="utf-8")
if "StructuredIpcProtocol.CapabilitySetDigest" not in worker_manager:
    fail("Worker launch and registration do not fence the capability digest")
