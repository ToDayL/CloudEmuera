...
expected_digest = hashlib.sha256(
    f"cloudemuera:{expected_matrix}:{expected_upstream}:structured-console-v3".encode()
).hexdigest()
...
if matrix.get("protocolVersion") != expected_protocol:
    fail("structured protocol version is not 3")
...
if "CurrentVersion = 3" not in protocol or "CapabilityMatrixVersion = \"p1-07\"" not in protocol:
    fail("v3 structured IPC constants are not frozen")
proto = (root / "src/CloudEmuera.Ipc/Protos/structured-worker.proto").read_text(encoding="utf-8")
for required_proto in ("package cloudemuera.ipc.v3;", "string capability_set_digest", "message ConsoleSnapshot", "message ConsoleTransaction"):
