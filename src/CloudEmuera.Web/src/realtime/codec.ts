...
const assetIdentifier = /^(?:path-[A-Za-z0-9_-]{1,2043}|[A-Za-z0-9._~-]{1,128})$/;
...
export function decodeRealtimeMessage(input: string | ArrayBuffer): RealtimeServerMessage {
  ...
  if (integer(envelope.protocolVersion, "protocolVersion") !== 6)
...
function decodeServerHello(value: JsonObject) {
  ...
  if (protocolVersion !== 6) throw new RealtimeDecodeError("unsupported_protocol_version", "服务端协议版本不兼容。");
...
function decodeLine(value: JsonObject): RealtimeLine {
  ensureKeys(value, ["lineId", "nodes", "alignment", "temporary", "noWrap", "layoutWidth", "lineHeight", "logicalLineId", "physicalIndex", "isLogicalStart"], "line");
  ...
  return { lineId: checkedIdentifier(value.lineId, "lineId"), nodes: array(value.nodes, "nodes").map(item => decodeNode(item)), alignment: alignment as RealtimeLine["alignment"], temporary: bool(value.temporary, "temporary"), noWrap: value.noWrap === undefined ? false : bool(value.noWrap, "noWrap"), layoutWidth: value.layoutWidth === undefined ? 0 : boundedInteger(value.layoutWidth, "layoutWidth", 0, 8192), lineHeight: value.lineHeight === undefined ? 0 : boundedInteger(value.lineHeight, "lineHeight", 0, 512), logicalLineId: value.logicalLineId === undefined || value.logicalLineId === null ? value.logicalLineId : checkedIdentifier(value.logicalLineId, "logicalLineId"), physicalIndex: value.physicalIndex === undefined ? 0 : boundedInteger(value.physicalIndex, "physicalIndex", 0, 4095), isLogicalStart: value.isLogicalStart === undefined ? true : bool(value.isLogicalStart, "isLogicalStart") };
}
...
function decodeNode(value: JsonValue): RealtimeNode {
  ...
  const allowed = {
    ...
    positionedInlineSegment: ["type", "children", "positionX", "measuredWidth", "action"],
  ...
  switch (type) {
    ...
    case "positionedInlineSegment": {
      ...
      return { type: "positionedInlineSegment", children: array(node.children, "children").map(decodeNode), positionX, measuredWidth, action };
    }
    case "image": return { type: "image", assetId: checkedAssetIdentifier(node.assetId, "assetId"), sourceRect: optionalRect(node.sourceRect, "sourceRect"), destination: optionalRect(node.destination, "destination"), altText: nullableString(node.altText, "altText"), decorative: bool(node.decorative, "decorative"), zIndex: integer(node.zIndex, "zIndex") };
    case "sprite": return { type: "sprite", assetId: checkedAssetIdentifier(node.assetId, "assetId"), sourceRect: decodeRect(object(node.sourceRect, "sourceRect")), destination: decodeRect(object(node.destination, "destination")), frame: nonNegativeInteger(node.frame, "frame"), zIndex: integer(node.zIndex, "zIndex"), opacity: boundedNumber(node.opacity, "opacity", 0, 1), altText: nullableString(node.altText, "altText"), hoverAssetId: nullableAssetIdentifier(node.hoverAssetId, "hoverAssetId"), hoverSourceRect: optionalRect(node.hoverSourceRect, "hoverSourceRect"), mappingAssetId: nullableAssetIdentifier(node.mappingAssetId, "mappingAssetId"), mappingSourceRect: optionalRect(node.mappingSourceRect, "mappingSourceRect"), animationFrames: array(node.animationFrames, "animationFrames").map(item => decodeAnimationFrame(object(item, "animationFrame"))) };
...
function decodeInlineAction(value: JsonObject) { ensureKeys(value, ["value", "tooltip", "enabled", "generation"], "action"); return { value: string(value.value, "action.value"), tooltip: nullableString(value.tooltip, "action.tooltip"), enabled: bool(value.enabled, "action.enabled"), generation: nonNegativeInteger(value.generation, "action.generation") }; }
...
function checkedAssetIdentifier(value: JsonValue | undefined, name: string): string { const result = string(value, name); if (!assetIdentifier.test(result)) throw new RealtimeDecodeError("invalid_identifier", `${name} 不是有效资产标识符。`); return result; }
function nullableAssetIdentifier(value: JsonValue | undefined, name: string): string | null | undefined { return value === null || value === undefined ? value : checkedAssetIdentifier(value, name); }
