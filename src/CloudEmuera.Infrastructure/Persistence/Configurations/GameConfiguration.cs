...
internal sealed class GameConfiguration : IEntityTypeConfiguration<GameRow>
{
    public void Configure(EntityTypeBuilder<GameRow> builder)
    {
        builder.ToTable(SqliteStorageConventions.GamesTable, table =>
        {
            ...
            table.HasCheckConstraint("ck_games_content", "(current_content_path IS NULL AND content_digest IS NULL AND content_revision = 0 AND activated_by IS NULL AND activated_at IS NULL) OR (current_content_path IS NOT NULL AND length(content_digest) = 71 AND substr(content_digest, 1, 7) = 'sha256:' AND lower(content_digest) = content_digest AND substr(content_digest, 8) NOT GLOB '*[^0-9a-f]*' AND length(substr(content_digest, 8)) = 64 AND content_revision > 0 AND activated_by IS NOT NULL AND activated_at IS NOT NULL)");
