...
- 支持玩家上传自己拥有的 ERB/CSV/资源游戏包，并在真实加载测试通过后自动启用为独立游戏。
...
- 本地账户或单一外部身份提供商登录。
- 游戏包一步上传、真实加载测试、自动启用与只读查看；不提供浏览器内 ERB/CSV 文件写入能力。
...
- **GAME-001**：系统必须通过单一步骤接收包含 `ERB`、`CSV`、配置及资源文件的 ZIP；每个上传包始终创建一个独立 Game，经真实加载测试通过后自动启用，不提供绑定到既有 Game 或手动验证/启用流程。
- **GAME-002**：上传阶段只检查归档可读性、archive/展开/单文件/entry/staging 容量，以及能否无歧义地安全物化到受保护 staging。不得以 Windows 保留名、大小写或 Unicode 规范化差异、目录布局、文件内容或编码策略拒绝上传；路径穿越、绝对路径、NUL、反斜杠歧义、精确重名和文件/目录冲突因无法安全物化仍必须拒绝。归档声明的链接或特殊文件只按普通文件/目录物化，不执行其链接或设备语义。
- **GAME-003**：带 BOM 的 UTF-16/UTF-32 文本必须尽力转为 UTF-8 并记录转换结果；其它文本可识别并记录编码，但未知、混合或无法转换的编码不得单独阻止上传或启用，最终兼容性由真实运行时加载结果决定。
- **GAME-004**：每个 Game 只保留一个当前可运行内容；该内容在被替换前必须不可变，并记录内容校验和、启用者、启用时间和运行时配置。系统不得提供 GameVersion、版本标签、版本列表或历史回滚资源。
- **GAME-005**：上传必须先写入本次新 Game 的内部 staging/workspace；真实加载通过后在同一操作内原子启用为 current content。该内部阶段不得公开为可绑定、可编辑或需用户确认的两步资源，也不得改变任何既有 Session 已复制的文件内容。
- **GAME-006**：系统必须提供已上传文件的目录浏览、文本只读查看和文件下载能力；浏览器内创建、编辑、重命名、删除或搜索 ERB/CSV 文件不在 MVP 范围内。
- **GAME-007**：兼容性检查只允许调用正式 Validator，以候选 staging/workspace 作为 GameRoot 执行真实运行时初始化加载；加载成功即允许自动启用，加载失败返回运行时诊断。不得再以全树编码正确、固定目录布局、静态入口点、缺失资源扫描、`CALLSHARP` 扫描或其它额外内容规则阻断启用。
...
- **AC-010**：路径穿越、绝对路径、NUL、反斜杠歧义或压缩炸弹包被拒绝且不写出 staging 根；归档链接元数据按普通内容物化，Windows 保留名、大小写/NFC 差异和非标准文本编码不在上传阶段被拒绝。
...
- WebSocket 重连、ConsoleSnapshot、存档隔离，以及固定 Emuera.EM+EE 基线全部 Supported 能力的
  结构化 Web 渲染和输入。
- 游戏包一步上传、内部 staging/workspace、真实加载测试与当前内容自动原子启用；不提供浏览器内文件写入工具或手动绑定/验证/启用流程。
