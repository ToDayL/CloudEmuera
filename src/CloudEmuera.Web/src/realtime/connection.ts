...
export class RealtimeConnectionManager {
  ...
  connect(): void {
    ...
    socket.addEventListener("open", () => {
      ...
      this.send("client.hello", { supportedProtocolVersions: [6], capabilityDigest: CAPABILITY_DIGEST, supportedCapabilities: [...SUPPORTED_CAPABILITIES] });
    });
  ...
  private send(type: string, payload: object, sessionId?: string, workerEpoch?: number): void {
    ...
    const message = { protocolVersion: 6, type, messageId: newMessageId(), ...(sessionId ? { sessionId } : {}), ...(workerEpoch ? { workerEpoch } : {}), payload };
