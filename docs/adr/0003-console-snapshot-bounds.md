...
- 默认上限由 `ConsoleContractLimits` 与 `ConsoleHistoryOptions` 集中声明：可见顶层节点 4096、可见文本 UTF-16 单位 262144、增量 1024 条、估算状态/历史 4 MiB、输入回执 2048 条。调用者可注入更小的值进行契约测试，但所有值必须为正。
