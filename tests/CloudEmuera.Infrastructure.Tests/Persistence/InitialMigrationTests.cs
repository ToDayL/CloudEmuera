...
public sealed class InitialMigrationTests
{
    ...
    public async Task EmptyDatabase_MigratesToLatestSchema()
    {
        ...
        Assert.Contains("font_size_line_height_mode", sessionColumns);
        Assert.Contains("session_identity_mode", sessionColumns);
        Assert.Contains("session_snapshot_id", sessionColumns);
        Assert.Equal(26, await ScalarIntAsync(scope.Connection, "SELECT COUNT(*) FROM schema_migrations;"));
    }
    ...
    public async Task LatestDatabase_MigrateAgain_IsNoOpAndPreservesRows()
    {
        ...
        Assert.Equal(26, await ScalarIntAsync(verify.Connection, "SELECT COUNT(*) FROM schema_migrations;"));
    }
    ...
    public async Task DatabaseNewerThanBinary_CheckFailsWithoutMutation()
    {
        ...
        Assert.Equal(27, await CountHistoryRowsAsync(database));
    }
