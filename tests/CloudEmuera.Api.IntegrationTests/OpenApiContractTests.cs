...
public sealed class OpenApiContractTests
{
    ...
    public async Task OpenApiDocumentExposesGameContractWithoutGameVersion()
    {
        ...
        Assert.True(paths.TryGetProperty("/api/v1/sessions/{sessionId}/presentation-manifest", out _), "OpenAPI must describe the session presentation manifest route.");
        ...
        AssertSchemaFields(schemas, "SessionPresentationManifest", "schemaVersion", "assets", "fonts", "fontDiagnostics");
        AssertSchemaFields(schemas, "SessionPresentationFont", "family", "assetId", "fallback", "cssFamily", "aliases");
        AssertSchemaFields(schemas, "SessionPresentationAsset", "assetId", "mediaType", "byteLength", "contentDigest", "eTag");
        Assert.DoesNotContain("GameVersion", json);
        Assert.DoesNotContain("game_version", json);
        Directory.Delete(dataRoot, recursive: true);
    }

    private static void AssertSchemaFields(JsonElement schemas, string name, params string[] expected)
    {
        Assert.True(schemas.TryGetProperty(name, out JsonElement schema), $"OpenAPI must expose schema {name}.");
        string[] actual = schema.GetProperty("properties").EnumerateObject().Select(property => property.Name).Order(StringComparer.Ordinal).ToArray();
