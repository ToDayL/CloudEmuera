...
internal sealed class ConfigData
{
...
#if CLOUDEMUERA_HEADLESS
	// CloudEmuera modification: the Simplified Chinese distribution at
	// https://gitee.com/Nahlot/emuera-cn emits these labels in emuera.config,
	// debug.config and _Replace.csv. Keep the aliases separate from the
	// Japanese/English display labels so saving a config still follows the
	// pinned upstream format. Entries are limited to ConfigCodes present in
	// this runtime; obsolete encoding-only options from that distribution are
	// intentionally not mapped to an unrelated setting.
	private static readonly IReadOnlyDictionary<string, ConfigCode> SimplifiedChineseConfigNames =
		new Dictionary<string, ConfigCode>(StringComparer.OrdinalIgnoreCase)
		{
	...
	private static readonly IReadOnlyDictionary<string, ConfigCode> SimplifiedChineseReplaceNames =
		new Dictionary<string, ConfigCode>(StringComparer.OrdinalIgnoreCase)
		{
	...
	private static readonly IReadOnlyDictionary<string, ConfigCode> SimplifiedChineseDebugNames =
		new Dictionary<string, ConfigCode>(StringComparer.OrdinalIgnoreCase)
		{
	...
	*/
	...
	public AConfigItem GetConfigItem(string key)
	{
		...
		key = key.ToUpper();
#if CLOUDEMUERA_HEADLESS
		if (SimplifiedChineseConfigNames.TryGetValue(key, out ConfigCode localizedCode))
			return GetConfigItem(localizedCode);
#endif
		foreach (AConfigItem item in configArray)
		{
			if (item == null)
	...
	public AConfigItem GetReplaceItem(string key)
	{
		...
		key = key.ToUpper();
#if CLOUDEMUERA_HEADLESS
		if (SimplifiedChineseReplaceNames.TryGetValue(key, out ConfigCode localizedCode))
			return GetReplaceItem(localizedCode);
#endif
		foreach (AConfigItem item in replaceArray)
		{
			if (item == null)
	...
	public AConfigItem GetDebugItem(string key)
	{
		...
		key = key.ToUpper();
#if CLOUDEMUERA_HEADLESS
		if (SimplifiedChineseDebugNames.TryGetValue(key, out ConfigCode localizedCode))
			return GetDebugItem(localizedCode);
#endif
		foreach (AConfigItem item in debugArray)
		{
			if (item == null)
