...
expected_integration_version="headless-p0.5.1"
...
grep -Fq "apt-get install -y --no-install-recommends libgdiplus" "$repo_root/Dockerfile"
grep -Fq "apt-get install -y --no-install-recommends libgdiplus" "$repo_root/deploy/docker/Dockerfile.dev"
