...
describe("RealtimeConnectionManager", () => {
  it("uses one socket, resumes by epoch, applies a continuous batch, and fences a gap", () => {
    ...
    socket.message({ protocolVersion: 3, type: "server.hello", messageId: "hello", payload: { protocolVersion: 3, payloadSchemaVersion: "p1-s03-display-commit", connectionId: "c1", serverNowUnixMilliseconds: Date.now(), heartbeatIntervalMilliseconds: 20_000, heartbeatTimeoutMilliseconds: 10_000, maxSubscriptionsPerConnection: 4, maxPendingInputsPerConnection: 32, serverMessageMaxBytes: 1_000_000, capabilityDigest: CAPABILITY_DIGEST } });
    expect(JSON.parse(socket.sent[1]).type).toBe("session.resume");
    socket.message({ protocolVersion: 3, type: "session.resume.result", messageId: "resume", sessionId: "s1", payload: { status: "ACCEPTED", workerEpoch: 3, reasonCode: null } });
    socket.message({ protocolVersion: 3, type: "session.snapshot", messageId: "snapshot", sessionId: "s1", workerEpoch: 3, sequence: 0, payload: { workerEpoch: 3, snapshotSequence: 0, committedFrameId: 0, consoleState: state } });
    expect(manager.getSessionState("s1")?.phase).toBe("snapshot_ready");
    socket.message({ protocolVersion: 3, type: "display.frame", messageId: "frame", sessionId: "s1", workerEpoch: 3, sequence: 1, payload: { workerEpoch: 3, frameId: 1, commitSequence: 1, reason: "EXPLICIT_REFRESH", requiresSnapshot: false, consoleState: null, transactions: [{ sequence: 1, operations: [{ type: "appendNodes", nodes: [{ type: "text", text: "hello", style: { decorations: [], fontFamily: "game-default", fontSize: 16, lineHeight: 20, foreground: null, background: null } }] }] }] } });
    expect(manager.getSessionState("s1")?.sequence).toBe(1);
    socket.message({ protocolVersion: 3, type: "display.frame", messageId: "gap", sessionId: "s1", workerEpoch: 3, sequence: 3, payload: { workerEpoch: 3, frameId: 2, commitSequence: 3, reason: "EXPLICIT_REFRESH", requiresSnapshot: false, consoleState: null, transactions: [{ sequence: 3, operations: [{ type: "clearConsole" }] }] } });
  ...
  it("marks a pending input unknown on disconnect instead of generating a new ID", () => {
    ...
    socket.message({ protocolVersion: 3, type: "server.hello", messageId: "hello", payload: { protocolVersion: 3, payloadSchemaVersion: "p1-s03-display-commit", connectionId: "c1", serverNowUnixMilliseconds: Date.now(), heartbeatIntervalMilliseconds: 20_000, heartbeatTimeoutMilliseconds: 10_000, maxSubscriptionsPerConnection: 4, maxPendingInputsPerConnection: 32, serverMessageMaxBytes: 1_000_000, capabilityDigest: CAPABILITY_DIGEST } });
    socket.message({ protocolVersion: 3, type: "session.snapshot", messageId: "snapshot", sessionId: "s1", workerEpoch: 3, sequence: 0, payload: { workerEpoch: 3, snapshotSequence: 0, committedFrameId: 0, consoleState: { ...state, currentPrompt: { promptId: "p1", inputType: "text", promptText: "Answer", defaultValue: null, constraints: { type: "text", maxLength: 20, minimum: null, maximum: null, allowSign: null, allowControlCharacters: null }, timeoutBehavior: "wait", timeoutAction: "close", allowedSources: ["keyboard"], oneInput: false, systemInput: false, stopMessageSkip: false, displayTime: false, timeoutMessage: null, openedAtUnixMilliseconds: Date.now(), deadlineUnixMilliseconds: Date.now() + 10_000, timeoutMilliseconds: 10_000 } }, } });
  ...
  it("transitions a subscribed stream to ended and fences later batches", () => {
    ...
    socket.open();
    socket.message({ protocolVersion: 3, type: "server.hello", messageId: "hello", payload: { protocolVersion: 3, payloadSchemaVersion: "p1-s03-display-commit", connectionId: "c1", serverNowUnixMilliseconds: Date.now(), heartbeatIntervalMilliseconds: 20_000, heartbeatTimeoutMilliseconds: 10_000, maxSubscriptionsPerConnection: 4, maxPendingInputsPerConnection: 32, serverMessageMaxBytes: 1_000_000, capabilityDigest: CAPABILITY_DIGEST } });
    socket.message({ protocolVersion: 3, type: "session.stream.ended", messageId: "ended", sessionId: "s1", workerEpoch: 3, payload: { reasonCode: "runtime_completed" } });
  ...
  it("waits for the replacement snapshot instead of unsubscribing during resync", () => {
    ...
    socket.message({ protocolVersion: 3, type: "server.hello", messageId: "hello", payload: { protocolVersion: 3, payloadSchemaVersion: "p1-s03-display-commit", connectionId: "c1", serverNowUnixMilliseconds: Date.now(), heartbeatIntervalMilliseconds: 20_000, heartbeatTimeoutMilliseconds: 10_000, maxSubscriptionsPerConnection: 4, maxPendingInputsPerConnection: 32, serverMessageMaxBytes: 1_000_000, capabilityDigest: CAPABILITY_DIGEST } });
    socket.message({ protocolVersion: 3, type: "session.snapshot", messageId: "snapshot", sessionId: "s1", workerEpoch: 3, sequence: 0, payload: { workerEpoch: 3, snapshotSequence: 0, committedFrameId: 0, consoleState: state } });
    socket.message({ protocolVersion: 3, type: "resync.required", messageId: "resync", sessionId: "s1", workerEpoch: 3, sequence: 1, payload: { workerEpoch: 3, observedSequence: 1, reason: "snapshot-replaced" } });
    expect(manager.getSessionState("s1")?.phase).toBe("resyncing");
    expect(socket.sent.filter(value => JSON.parse(value).type === "session.unsubscribe")).toHaveLength(0);
    socket.message({ protocolVersion: 3, type: "session.snapshot", messageId: "replacement", sessionId: "s1", workerEpoch: 3, sequence: 1, payload: { workerEpoch: 3, snapshotSequence: 1, committedFrameId: 1, consoleState: state } });
    expect(manager.getSessionState("s1")?.phase).toBe("snapshot_ready");
    manager.dispose();
    vi.unstubAllGlobals();
  });
  ...
  it("does not mutate the console state for an unsubscribe acknowledgement", () => {
    ...
    socket.message({ protocolVersion: 3, type: "server.hello", messageId: "hello", payload: { protocolVersion: 3, payloadSchemaVersion: "p1-s03-display-commit", connectionId: "c1", serverNowUnixMilliseconds: Date.now(), heartbeatIntervalMilliseconds: 20_000, heartbeatTimeoutMilliseconds: 10_000, maxSubscriptionsPerConnection: 4, maxPendingInputsPerConnection: 32, serverMessageMaxBytes: 1_000_000, capabilityDigest: CAPABILITY_DIGEST } });
    socket.message({ protocolVersion: 3, type: "session.snapshot", messageId: "snapshot", sessionId: "s1", workerEpoch: 3, sequence: 0, payload: { workerEpoch: 3, snapshotSequence: 0, committedFrameId: 0, consoleState: state } });
    socket.message({ protocolVersion: 3, type: "session.stream.ended", messageId: "ended", sessionId: "s1", workerEpoch: 3, payload: { reasonCode: "unsubscribed" } });
    expect(manager.getSessionState("s1")?.phase).toBe("snapshot_ready");
    manager.dispose();
    vi.unstubAllGlobals();
  });
  ...
  it("retries a not-ready snapshot with a bounded delay", () => {
    ...
    socket.open();
    socket.message({ protocolVersion: 3, type: "server.hello", messageId: "hello", payload: { protocolVersion: 3, payloadSchemaVersion: "p1-s03-display-commit", connectionId: "c1", serverNowUnixMilliseconds: Date.now(), heartbeatIntervalMilliseconds: 20_000, heartbeatTimeoutMilliseconds: 10_000, maxSubscriptionsPerConnection: 4, maxPendingInputsPerConnection: 32, serverMessageMaxBytes: 1_000_000, capabilityDigest: CAPABILITY_DIGEST } });
    socket.message({ protocolVersion: 3, type: "session.resume.result", messageId: "resume-1", sessionId: "s1", payload: { status: "SNAPSHOT_NOT_READY", workerEpoch: null, reasonCode: "snapshot_pending" } });
