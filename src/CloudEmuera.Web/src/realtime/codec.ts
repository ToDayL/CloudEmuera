...
function decodeLine(value: JsonObject): RealtimeLine {
  ensureKeys(value, ["lineId", "nodes", "alignment", "temporary"], "line");
  ...
  return { lineId: checkedIdentifier(value.lineId, "lineId"), nodes: array(value.nodes, "nodes").map(item => decodeNode(item)), alignment: alignment as RealtimeLine["alignment"], temporary: bool(value.temporary, "temporary") };
}
...
function decodeNode(value: JsonValue): RealtimeNode {
  ...
  const allowed = {
    ...
    lineBreak: ["type"],
    button: ["type", "children", "value", "tooltip", "enabled", "generation"],
    ...
    shape: ["type", "shape", "bounds", "fill", "stroke", "zIndex", "points"],
    htmlIsland: ["type", "root", "layout"],
  ...
  switch (type) {
    ...
    case "button": return { type: "button", children: array(node.children, "children").map(decodeNode), value: string(node.value, "value"), tooltip: nullableString(node.tooltip, "tooltip"), enabled: bool(node.enabled, "enabled"), generation: nonNegativeInteger(node.generation, "generation") };
    ...
    case "shape": return { type: "shape", shape: shapeKind(node.shape), bounds: decodeRect(object(node.bounds, "bounds")), fill: optionalColor(node.fill, "fill"), stroke: optionalColor(node.stroke, "stroke"), zIndex: integer(node.zIndex, "zIndex"), points: array(node.points, "points").map(item => decodePoint(object(item, "point"))) };
    case "htmlIsland": return { type: "htmlIsland", root: decodeHtml(object(node.root, "root")), layout: optionalRect(node.layout, "layout") };
...
function decodeStyle(value: JsonObject): RealtimeTextStyle { ensureKeys(value, ["decorations", "fontFamily", "fontSize", "lineHeight", "foreground", "background"], "style"); return { decorations: array(value.decorations, "decorations").map(item => string(item, "decoration")), fontFamily: string(value.fontFamily, "fontFamily"), fontSize: boundedNumber(value.fontSize, "fontSize", 1, 256), lineHeight: boundedNumber(value.lineHeight, "lineHeight", 0, 512), foreground: optionalColor(value.foreground, "foreground"), background: optionalColor(value.background, "background") }; }
