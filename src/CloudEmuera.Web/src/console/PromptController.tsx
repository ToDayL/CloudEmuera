...
export function PromptController({ prompt, disabled, pending, serverTimeOffsetMilliseconds, onInput }: { prompt?: Prompt | null; disabled?: boolean; pending?: boolean; serverTimeOffsetMilliseconds: number; onInput: (event: ConsoleInputEvent) => void }) {
  ...
  const controlsDisabled = Boolean(disabled || pending || deadlineExpired || prompt.inputType === "waitOnly");
  ...
  const showInputForm = !["anyKey", "primitivePointerKey"].includes(prompt.inputType);
