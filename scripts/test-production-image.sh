...
class WebSocket:
    def __init__(self):
        ...
        request = (
            ...
            "Sec-WebSocket-Protocol: cloudemuera.realtime.v3\r\n"
            "Origin: http://localhost:5173\r\n"
            f"Cookie: {cookie_header(cookie_path)}\r\n\r\n"
        ).encode()
        self.sock.sendall(request)
        header = self.read_until(b"\r\n\r\n").decode("latin1")
        if " 101 " not in header or "Sec-WebSocket-Protocol: cloudemuera.realtime.v3" not in header:
...
digest = hashlib.sha256(b"cloudemuera:p1-07:2175f8a629257efb08214e093704b3a3d3d06d05:structured-console-v5-display-commit").hexdigest()
socket_client = WebSocket()
try:
    socket_client.send({"protocolVersion": 3, "type": "client.hello", "messageId": "production-hello", "payload": {
        "supportedProtocolVersions": [3], "capabilityDigest": digest, "supportedCapabilities": []}})
    ...
    for attempt in range(40):
        socket_client.send({"protocolVersion": 3, "type": "session.resume", "messageId": f"production-resume-{attempt}",
    ...
    socket_client.send({"protocolVersion": 3, "type": "session.input", "messageId": "production-input-envelope",
