...
./scripts/verify-third-party.sh
./scripts/dev-up.sh
curl --fail http://localhost:28648/health/live
...
实施记录（2026-08-18）：输入框为空且 prompt 允许键盘输入时，玩家在控制台输出区左键或触摸点击非按钮区域会提交空值 `KEYBOARD` Enter；选项、画布命中区、声音和滚动等按钮及表单控件不触发该快捷交互。新增输入控制器和点击目标过滤回归测试，未改变 realtime/IPC 协议。

实施记录（2026-08-18）：`SETBGCOLOR`/`RESETBGCOLOR` 通过已有 `windowMetadata.defaultBackground` 表达，并由控制台主区域统一着色；文字节点不再重复携带全局背景色，输出和画布层保持透明，避免文字底部出现背景色块。新增 RuntimeBridge 与控制台背景样式回归测试，未改变 realtime 协议结构。

实施记录（2026-08-19）：控制台保留快照截断元数据但不再向玩家显示“输出过长”诊断条；没有背景、drawable 或命中区的空画布不再渲染带边框的空框，避免输入回执下方出现无内容区域。新增空画布内容判定回归测试，未改变 realtime 协议结构。
