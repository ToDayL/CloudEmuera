...
expected_matrix = "p1-s09"
expected_runtime = "headless-p0.5.1"
expected_protocol = 7
expected_digest = hashlib.sha256(
    f"cloudemuera:{expected_matrix}:{expected_upstream}:structured-console-v7-authoritative-layout".encode()
).hexdigest()
...
if matrix.get("matrixVersion") != expected_matrix:
    fail("matrixVersion is not p1-s09")
...
if matrix.get("protocolVersion") != expected_protocol:
    fail("structured protocol version is not 7")
...
if "CurrentVersion = 7" not in protocol or "CapabilityMatrixVersion = \"p1-s09\"" not in protocol:
    fail("v7 structured IPC constants are not frozen")
proto = (root / "src/CloudEmuera.Ipc/Protos/structured-worker.proto").read_text(encoding="utf-8")
for required_proto in ("package cloudemuera.ipc.v7;", "string capability_set_digest", "message ConsoleSnapshot", "message ConsoleTransaction", "message TooltipPresentation", "message TooltipResource"):
